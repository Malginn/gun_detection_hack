...
function uploadFile() {
    ...
    if (file) {
        ...
        if(fileExtension === 'zip')
        {
            ...
            fetch('http://62.84.112.66:8000/api/archive/', options)
        ...
        {
            // Выполняем запрос к серверу
            fetch('http://62.84.112.66:8000/api/nn/', options)
                .then(response => response.json())
                .then(data => {
                    check(data['redis_key'])
                })
...
function check (task) {
    ...
    fetch('http://62.84.112.66:8000/api/task/?task_key='+task, options)
        .then(response => response.json())
        .then(data => {
            ...
            if (data['status'] !== 'done') {
                ...
                if (filename.match(/\.(mp4)$/)) {
                    console.log('video')
                    preview.innerHTML = '<video src="http://62.84.112.66:8000/api/video/?task_key='+task+'" width="80%" controls autoplay></video>';
                } else if (filename.match(/\.(jpg|jpeg|png)$/)) {
                    console.log('file')
                    preview.innerHTML = '<img src="http://62.84.112.66:8000/api/media/?task_key='+task+'" style="width: 100%; border-radius: 13px;">';
                }
            }
        })
        .catch(error => {
